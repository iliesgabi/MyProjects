...
public class Admin extends ConClass {
	...
	public Admin(String cnp) {
		...
		slPanel.putConstraint(SpringLayout.NORTH, addPoliclinicaBtnPanel, 130, SpringLayout.NORTH, leftPanel);
		...
		slPanel.putConstraint(SpringLayout.NORTH, addUserBtnPanel, 230, SpringLayout.NORTH, leftPanel);
		...
		slPanel.putConstraint(SpringLayout.NORTH, modifyUserBtnPanel, 330, SpringLayout.NORTH, leftPanel);
		...
		slPanel.putConstraint(SpringLayout.NORTH, deleteUserBtnPanel, 430, SpringLayout.NORTH, leftPanel);
	...
	private class AddAngajatListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			numarOreTotalLabel = new JLabel("numar de ore de munca totale: ");
			...
			slPanel.putConstraint(SpringLayout.WEST, numarOreTotalLabel, 520, SpringLayout.WEST, leftPanel);
	...
	private class ProgramListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			try {
				SqlProcedures.creareProgramUnitate(con, Integer.parseInt(id.getText()), anP.getText(), lunaP.getText(),
						ziP.getText(), start_p.getText(), end_p.getText());
			} catch (NumberFormatException | SQLException e1) {
				e1.printStackTrace();

				errorMessage = new JLabel("date incorecte ");
				errorMessage.setFont(Constants.Fonts.font_label);
				slPanel.putConstraint(SpringLayout.NORTH, errorMessage, 100, SpringLayout.NORTH, leftPanel);
				slPanel.putConstraint(SpringLayout.WEST, errorMessage, 760, SpringLayout.WEST, leftPanel);
				leftPanel.add(errorMessage);
			}
			...
			try {
				...
				while (rs.next()) {
					if (rs.getString("functie").equals("asistent medical")) {
						AsistentMedical a = new AsistentMedical(cnpPeBune, true, false);
					} else if (rs.getString("functie").equals("medic")) {
						 Medic a = new Medic(cnpPeBune, true,false);
					} else if (rs.getString("functie").equals("inspector")) {
						 Inspector a = new Inspector(cnpPeBune, true,false);
					} else if (rs.getString("functie").equals("expert")) {
						 Expert a = new Expert(cnpPeBune, true,false);
					} else if (rs.getString("functie").equals("receptioner")) {
						 Receptioner a = new Receptioner(cnpPeBune, true,false);
					}
				}
			} catch (SQLException e1) {
				e1.printStackTrace();
